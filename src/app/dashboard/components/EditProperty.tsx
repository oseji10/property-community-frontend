...
export default function EditProperty() {
      ...
      try {
        ...
        setCurrencies(
          data.sort((a: Currency, b: Currency) =>
            a.currencyCode === 'NGN' ? -1 : b.currencyCode === 'NGN' ? 1 : 0
          )
        );
      } catch (err: any) {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        // Populate form data
        setFormData({
          propertyTitle: fetchedProperty.propertyTitle || '',
          listingType: fetchedProperty.listingType || '',
          price: String(fetchedProperty.price || ''),
          currency: String(
            typeof fetchedProperty.currency === 'object'
              ? fetchedProperty.currency.currencyId
              : fetchedProperty.currency || ''
          ),
          bedrooms: fetchedProperty.bedrooms != null ? String(fetchedProperty.bedrooms) : '',
          bathrooms: fetchedProperty.bathrooms != null ? String(fetchedProperty.bathrooms) : '',
          garage: fetchedProperty.garage || '',
          ...
          propertyTypeId: String(fetchedProperty.propertyTypeId || ''),
          amenities: fetchedProperty.amenities || '',
          otherFeatures: fetchedProperty.otherFeatures || '',
          propertyDescription: fetchedProperty.propertyDescription || '',
        });
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formPayload.append('propertyTitle', formData.propertyTitle);
      ...
      formPayload.append('garage', formData.garage);
      formPayload.append('state', formData.state);
      formPayload.append('city', formData.city);
      formPayload.append('address', formData.address);
      formPayload.append('latitude', formData.latitude);
      formPayload.append('longitude', formData.longitude);
      formPayload.append('propertyTypeId', formData.propertyTypeId);
      formPayload.append('amenities', formData.amenities);
      formPayload.append('otherFeatures', formData.otherFeatures);
      formPayload.append('propertyDescription', formData.propertyDescription);

      selectedFiles.forEach((file) => formPayload.append('images', file));

      const response = await api.post(`/properties/${slug}/edit?_method=PUT`, formPayload, {
